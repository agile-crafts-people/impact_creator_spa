# Creators Dashboard — Creator SPA

This repository contains a Vue 3 single-page application (SPA) for the creator service.
...
- Developer [SPA Standard Prerequisites](https://github.com/agile-crafts-people/impact/blob/main/DeveloperEdition/standards/spa_standards.md)
...
**Note**: This template uses `@agile-crafts-people/impact_spa_utils` for reusable components, composables, and utilities. See the [impact_spa_utils README](../impact_spa_utils/README.md) for complete documentation on available components (`AutoSaveField`, `AutoSaveSelect`, `ListPageSearch`), composables (`useResourceList`, `useErrorHandler`, `useRoles`), and utilities (`formatDate`, `validationRules`).
...
This template uses components and composables from `@agile-crafts-people/impact_spa_utils`:
